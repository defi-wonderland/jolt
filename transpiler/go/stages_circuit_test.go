...
// TestCrossSignerLamport proves two different Lamport key pairs using the same cached
// pk/vk — no re-setup needed. Then generates a Foundry test that calls verifyProof
// twice on the same deployed JoltVerifier_S.sol contract, demonstrating that the
// circuit is universal: it encodes how Lamport verification works, not which keys.
//
// Requires two witness files in the active class directory:
//
//	stages_witness_1.json  (signer 1: SEED_1, MSG_1)
//	stages_witness_2.json  (signer 2: SEED_2, MSG_2)
//
// Produce them by running the transpiler for each signer and copying:
//
//	cargo run -p lamport-sig ... -- --save --signer 1
//	cargo run -p transpiler ... -- --proof /tmp/lamport_proof_1.bin --io-device /tmp/lamport_io_device_1.bin
//	cp transpiler/go/class_S/stages_witness.json transpiler/go/class_S/stages_witness_1.json
//
//	cargo run -p lamport-sig ... -- --save --signer 2
//	cargo run -p transpiler ... -- --proof /tmp/lamport_proof_2.bin --io-device /tmp/lamport_io_device_2.bin
//	cp transpiler/go/class_S/stages_witness.json transpiler/go/class_S/stages_witness_2.json
//
//	go test -v -run TestCrossSignerLamport -timeout 60m
func TestCrossSignerLamport(t *testing.T) {
	t.Log("Jolt Stages 1-7 Verifier - Cross-Signer Lamport Verification")

	exampleName := "lamport-sig"
	...
	if classDir == "" {
		t.Fatal("No class directory with stages_witness_1.json found — run the transpiler for each signer first")
	}
	t.Logf("Auto-detected class directory: class_%s", className)
	...
	t.Logf("Loaded witness 1 (signer 1: SEED_1, MSG_1=0xdeadbeefcafef00d)")

	assignment2, err := LoadStagesAssignment(filepath.Join(classDir, "stages_witness_2.json"))
	if err != nil {
		t.Fatalf("Failed to load witness 2: %v", err)
	}
	t.Logf("Loaded witness 2 (signer 2: SEED_2, MSG_2=0x0badf00dcafebabe)")
	...
	proof1, in1 := proveOne("signer 1 (SEED_1, MSG_1)", assignment1)
	proof2, in2 := proveOne("signer 2 (SEED_2, MSG_2)", assignment2)
...
// Cross-signer verification: two different Lamport key pairs verified
// by the same JoltVerifier_%s.sol (shared Groth16 trusted setup).
// The circuit encodes how Lamport verification works, not which keys were used.
contract JoltVerifierCrossSignerTest is Test {
    Verifier verifier;
    ...
    // Signer 1: key pair derived from SEED_1, message=0xdeadbeefcafef00d
    function test_%s_signer1() public view {
        uint256[8] memory proof;
%s
...
%s
    ...
    // Signer 2: key pair derived from SEED_2, message=0x0badf00dcafebabe
    function test_%s_signer2() public view {
        uint256[8] memory proof;
%s
...
`,
	...
	testPath := filepath.Join(foundryDir, "test", "JoltVerifierCrossSigner.t.sol")
	if err := os.WriteFile(testPath, []byte(foundryTest), 0644); err != nil {
		t.Fatalf("Write Foundry test: %v", err)
	}
	...
	t.Log("=== Summary ===")
	t.Logf("Circuit:         %d constraints (class %s, universal)", compiledR1cs.GetNbConstraints(), className)
	t.Logf("Signer 1 proof:  164 bytes  ✓  (SEED_1, MSG_1=0xdeadbeefcafef00d)")
	t.Logf("Signer 2 proof:  164 bytes  ✓  (SEED_2, MSG_2=0x0badf00dcafebabe)")
	t.Logf("Contract:        %s (same vk, both proofs valid)", solFileName)
	t.Logf("Foundry test:    %s", testPath)
	t.Log("")
	t.Log("=== Next steps ===")
	t.Logf("  cd %s", foundryDir)
	t.Log("  forge test -vv --match-contract JoltVerifierCrossSignerTest")
	t.Log("")
	t.Log("Both test_lamport_sig_signer1() and test_lamport_sig_signer2() should pass,")
	t.Log("verifying two different Lamport key pairs against the same deployed contract.")
}
