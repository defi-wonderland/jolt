...
fn main() {
    ...
    // input_words as symbolic variables from symbolize_io_device.
    let bytecode_words_concrete = &real_preprocessing.shared.ram.bytecode_words;
    let bytecode_words_ast: Vec<MleAst> = bytecode_words_concrete
        .iter()
        .map(|&w| MleAst::from(w as u128))
        .collect();
