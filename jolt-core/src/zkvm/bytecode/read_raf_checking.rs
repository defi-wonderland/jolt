    ...
    SumcheckInstanceVerifier<F, T, A> for BytecodeReadRafSumcheckVerifier<F>
{
    ...
    fn expected_output_claim(&self, accumulator: &A, sumcheck_challenges: &[F::Challenge]) -> F {
        ...
        let val_evals: [F; N_STAGES] = if let Some(overrides) = take_pending_val_poly_evals::<F>() {
            overrides
        } else {
            let evals: [F; N_STAGES] =
                array::from_fn(|s| self.params.val_polys[s].evaluate(&r_address_prime.r));
        ...
        let val = val_evals
            ...
            .map(|(((val_eval, r_cycle), gamma), int_poly)| {
                (*val_eval + int_poly) * EqPolynomial::<F>::mle(r_cycle, &r_cycle_prime.r) * gamma
            })
            .sum::<F>();
