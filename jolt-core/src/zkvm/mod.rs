...
///
/// With `padded-io` feature: inputs/outputs are zero-padded to max_input_size /
/// max_output_size before hashing. This ensures all executions of the same program
/// produce the same transcript structure (same number of Poseidon hashes), which
/// is required for circuit universality (shared trusted setup across executions).
